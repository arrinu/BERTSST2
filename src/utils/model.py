...
class ClassificationHead(nn.Module):
    ...
    def forward(self,x):
        x = torch.mean(x, dim=1) # Batch, Seq, d_model -> Batch, d_model
        return torch.log_softmax(self.proj(x), dim =-1) # Batch, d_model -> Batch, Num_labels
...
def build_bert(src_vocab_size, label_vocab_size, src_seq_len, d_model, N, h, dropout, intermediate_size):
    ...
    model = BERT(encoder, src_embed, src_pos,  classification_head)
